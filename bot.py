...
async def process_ai_result(message: Message, state: FSMContext, ai_result, source_label: str):
    ...
    matched_items = match_tests(ai_result.tests, price_list)
    await state.update_data(
        matched_items=matched_items,
        candidate_limits={},
        ai_usage=ai_result.usage.to_dict(),
        ai_raw_text=ai_result.raw_text,
        ai_source=source_label,
    )

    recognized = "\n".join(f"{i}. {test}" for i, test in enumerate(ai_result.tests, start=1))
    await message.answer(f"Распознал список анализов:\n\n{recognized}")
...
async def any_text_starts_order(message: Message, state: FSMContext):
    ...
    await state.update_data(matched_items=matched_items, candidate_limits={}, ai_usage=None, ai_raw_text=None, ai_source=None)
    await show_review(message, state)
