...
def get_preview_keyboard(matched_items):
    if has_ambiguous_items(matched_items):
        return candidate_selection_keyboard(matched_items)
    return confirm_list_keyboard(has_ambiguous=False)


async def show_matched_list(message_or_callback_message, matched_items):
    await message_or_callback_message.answer(
        format_preview_message(matched_items),
        reply_markup=get_preview_keyboard(matched_items),
    )
...
async def any_text_starts_order(message: Message, state: FSMContext):
    ...
    await show_matched_list(message, matched_items)
...
async def choose_candidate(callback: CallbackQuery, state: FSMContext):
    ...
    await show_matched_list(callback.message, matched_items)
    await callback.answer()
