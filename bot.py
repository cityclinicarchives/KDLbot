...
from ai_service import (
    is_ai_enabled,
    extract_tests_from_audio_file,
    extract_tests_from_image_file,
    match_user_request_with_ai,
    remember_manual_choice,
)
...
async def start_new_search(message: Message, state: FSMContext, text: str | None = None):
    await state.clear()
    await state.set_state(OrderFlow.waiting_for_tests)
    await message.answer(
        text or (
            "Здравствуйте! Отправьте мне список анализов текстом, голосом или фото.\n\n"
            "Например текстом:\n"
            "ОАК\n"
            "ТТГ\n"
            "витамин Д\n\n"
...
async def download_telegram_file(file_id: str, suffix: str) -> str:
...
async def process_raw_text_order(message: Message, state: FSMContext, raw_text: str, source_label: str, initial_usage=None):
    if not raw_text or not raw_text.strip():
        await message.answer(
            "Не удалось распознать список анализов. Попробуйте отправить фото/голос еще раз "
            "или напишите список анализов текстом."
        )
        return

    if not is_ai_enabled():
        user_tests = split_user_tests(raw_text)
        matched_items = match_tests(user_tests, price_list)
        await state.update_data(matched_items=matched_items, candidate_limits={}, ai_usage=None, ai_raw_text=raw_text, ai_source=None)
        await show_review(message, state)
        return

    await message.answer("Обрабатываю список анализов и подбираю позиции в прайс-листе.")
    ai_match = await match_user_request_with_ai(
        raw_text=raw_text,
    ...
    if not ai_match.matched_items:
        await message.answer(
            "Не удалось подобрать анализы. Попробуйте написать список иначе или отправить фото/голос еще раз."
        )
        return

    await state.update_data(
        matched_items=ai_match.matched_items,
        candidate_limits={},
        ai_usage=ai_match.usage.to_dict(),
        ai_raw_text=raw_text,
        ai_cleaned_items=ai_match.cleaned_items,
        ai_source=source_label,
    )

    if source_label in {"voice", "photo", "document"}:
        recognized = raw_text.strip()
        if len(recognized) > 1000:
            recognized = recognized[:1000] + "..."
        await message.answer(f"Распознал текст:\n\n{recognized}")

    await show_review(message, state)


async def process_ai_result(message: Message, state: FSMContext, ai_result, source_label: str):
    await process_raw_text_order(
        message=message,
        state=state,
        raw_text=ai_result.raw_text,
        source_label=source_label,
        initial_usage=ai_result.usage,
    )
...
async def handle_voice_message(message: Message, state: FSMContext):
    if not is_ai_enabled():
        await message.answer(
            "Распознавание голосовых сообщений пока не настроено: добавьте OPENAI_API_KEY в .env или Railway Variables."
        )
        return

    await message.answer("Получил голосовое сообщение. Распознаю список анализов.")
    path = None
    try:
        path = await download_telegram_file(message.voice.file_id, ".ogg")
        ai_result = await extract_tests_from_audio_file(path)
        await process_ai_result(message, state, ai_result, "voice")
    except Exception as exc:
        logger.exception("Ошибка распознавания голосового сообщения")
        await message.answer(f"Не удалось распознать голосовое сообщение. Ошибка: {exc}")
    finally:
...
async def handle_photo_message(message: Message, state: FSMContext):
    if not is_ai_enabled():
        await message.answer(
            "Распознавание изображений пока не настроено: добавьте OPENAI_API_KEY в .env или Railway Variables."
        )
        return

    await message.answer("Получил изображение. Распознаю список анализов.")
    path = None
    try:
        photo = message.photo[-1]
        path = await download_telegram_file(photo.file_id, ".jpg")
        ai_result = await extract_tests_from_image_file(path)
        await process_ai_result(message, state, ai_result, "image")
    except Exception as exc:
        logger.exception("Ошибка распознавания изображения")
        await message.answer(f"Не удалось распознать изображение. Ошибка: {exc}")
    finally:
        if path and os.path.exists(path):
            os.remove(path)
...
async def handle_image_document(message: Message, state: FSMContext):
    if not message.document or not (message.document.mime_type or "").startswith("image/"):
        await message.answer("Отправьте изображение как фото или документ-изображение.")
        return
    ...
    await message.answer("Получил изображение-файл. Распознаю список анализов.")
    path = None
    suffix = os.path.splitext(message.document.file_name or "")[1] or ".jpg"
    try:
        path = await download_telegram_file(message.document.file_id, suffix)
        ai_result = await extract_tests_from_image_file(path)
        await process_ai_result(message, state, ai_result, "image_document")
    except Exception as exc:
        logger.exception("Ошибка распознавания изображения-документа")
        await message.answer(f"Не удалось распознать изображение. Ошибка: {exc}")
    finally:
        if path and os.path.exists(path):
            os.remove(path)
...
async def cmd_reset(message: Message, state: FSMContext):
    await start_new_search(message, state, "Диалог сброшен. Отправьте список анализов текстом, голосом или фото.")


@dp.message(F.text == "🔍 Новый поиск")
async def new_search_button(message: Message, state: FSMContext):
    await start_new_search(message, state)


@dp.message(StateFilter(None), F.voice)
@dp.message(OrderFlow.waiting_for_tests, F.voice)
...
@dp.message(OrderFlow.waiting_for_tests, F.photo)
async def receive_photo(message: Message, state: FSMContext):
    await handle_photo_message(message, state)


@dp.message(StateFilter(None), F.document)
@dp.message(OrderFlow.waiting_for_tests, F.document)
async def receive_image_document(message: Message, state: FSMContext):
    await handle_image_document(message, state)
...
async def any_text_starts_order(message: Message, state: FSMContext):
    ...
    await process_raw_text_order(message, state, message.text, "text", initial_usage=None)


@dp.callback_query(F.data == "restart")
async def restart(callback: CallbackQuery, state: FSMContext):
    await start_new_search(callback.message, state, "Начнем заново. Отправьте список анализов текстом, голосом или фото.")
...
async def receive_tests(message: Message, state: FSMContext):
    if not message.text:
        await message.answer("Пока на этом этапе я принимаю список анализов только текстом, голосом или изображением.")
        return
...
async def choose_edit_candidate(callback: CallbackQuery, state: FSMContext):
    ...
    remember_manual_choice(item.input_name, selected_candidate.code, selected_candidate.matched_name)

    active_group = getattr(item, "active_group", "") or getattr(selected_candidate, "active_group", "")
    if active_group:
        try:
            learn_group_priority(active_group, selected_candidate.code)
            global price_list
            price_list = load_price_list()
        except Exception as exc:
            logger.warning("Не удалось обновить самообучение приоритетов: %s", exc)
...
async def receive_delete_numbers(message: Message, state: FSMContext):
    ...
    if not matched_items:
        ...
        await state.set_state(OrderFlow.waiting_for_tests)
        await message.answer(
            "Список стал пустым. Отправьте новый список анализов текстом, голосом или фото.",
            reply_markup=main_reply_keyboard(),
        )
        return
...
async def receive_contact(message: Message, state: FSMContext):
    ...
    lab_order_text = format_order_for_lab(
        order_id,
        patient_name,
        phone,
        patient_discount,
    ...
    await message.answer("Ваш заказ принят.\n\n" + order_text, reply_markup=main_reply_keyboard())

    if LAB_CHAT_ID != 0:
        await bot.send_message(LAB_CHAT_ID, lab_order_text)
    else:
        await message.answer(
...
async def fallback(message: Message, state: FSMContext):
    await state.set_state(OrderFlow.waiting_for_tests)
    await message.answer(
        "Я готов принять список анализов текстом, голосом или фото.",
        reply_markup=main_reply_keyboard(),
    )
