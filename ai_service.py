...
def _find_by_alias_groups_fuzzy(query: str, price_list: list[LabTest], limit: int = 8) -> list[CandidateItem]:
    ...
    group_items = [item for item in price_list if item.group == best_group or getattr(item, "group2", "") == best_group]
    if not group_items:
        return []
    ...
    def active_priority(item):
        if item.group == best_group:
            return item.priority
        if getattr(item, "group2", "") == best_group:
            return getattr(item, "priority2", 0)
        return item.priority

    scored.sort(key=lambda pair: (-active_priority(pair[1]), -pair[0], pair[1].price, pair[1].name))
    return [labtest_to_candidate(item, min(100, score), active_group=best_group) for score, item in scored[:limit]]
