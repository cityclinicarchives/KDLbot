...
def load_price_list(path: str = "price_list.xlsx") -> list[LabTest]:
    """
    Единый источник данных: только price_list.xlsx.

    Комплексы больше НЕ читаются из complex_price_list.xlsx.
    Если строка является комплексом, в price_list.xlsx должен быть заполнен столбец components.
    Для одиночных анализов components должен быть пустым.
    """
...
def get_active_priority(item: LabTest, active_group: str = "") -> tuple[int, str]:
    """Return priority value and field name for the group that matched the request."""
    group = str(active_group or "").strip()
    if group:
        if item.group == group:
...
def item_in_group(item: LabTest, group: str) -> bool:
    return bool(group) and (item.group == group or item.group2 == group)


def labtest_to_candidate(item: LabTest, confidence: int, active_group: str = "") -> CandidateItem:
    active_priority, active_field = get_active_priority(item, active_group)
    active_group_value = active_group or item.group
    return CandidateItem(
        ...
        active_group=active_group_value,
        active_priority_field=active_field,
...
def _sort_group_items(items: list[LabTest], active_group: str = "") -> list[LabTest]:
    return sorted(items, key=lambda item: (-get_active_priority(item, active_group)[0], item.name))
...
def _find_by_alias_group(query: str, price_list: list[LabTest], max_candidates: int) -> list[CandidateItem]:
    ...
    group_items = [item for item in price_list if item_in_group(item, group)]
    ...
    def group_sort_key(item: LabTest):
        ...
        active_priority, _ = get_active_priority(item, group)
    ...
    candidates = [labtest_to_candidate(item, max(100, get_score_for_item(query, query, item)), active_group=group) for item in group_items]
...
def learn_group_priority(active_group: str, selected_code: str, price_list_path: str = "price_list.xlsx") -> bool:
    """
    Self-learning priority update for the active group.

    Rules:
    - priority 100 is manual and never changed automatically;
    - selected item with priority <100 gets +1, capped at 90;
    - other items in the active group with priority <100 get -1, floored at 10;
    - if active group matched group2, update priority2 only.
    """
    ...
    code_col = ensure_column("code")
    group_col = ensure_column("group")
    priority_col = ensure_column("priority")
    group2_col = ensure_column("group2")
    priority2_col = ensure_column("priority2")

    touched = False
    for row in range(2, sheet.max_row + 1):
        code = str(sheet.cell(row, code_col).value or "").strip()
        group = str(sheet.cell(row, group_col).value or "").strip()
        group2 = str(sheet.cell(row, group2_col).value or "").strip()

        if group == active_group:
            pr_col = priority_col
        elif group2 == active_group:
            pr_col = priority2_col
        else:
            continue

        current = parse_int(sheet.cell(row, pr_col).value, default=0)
        if current >= 100:
            continue

        if code == selected_code:
            new_value = min(90, max(10, current + 1))
        else:
            new_value = max(10, current - 1)

        if new_value != current:
            sheet.cell(row, pr_col, new_value)
            touched = True
