...
def load_alias_groups(path: str = "alias_groups.xlsx") -> dict[str, str]:
    """
    Читает файл alias_groups.xlsx.

    Ожидаемый лист: первый лист книги.
    Формат колонок:
    alias, group
    оак, cbc
...
def load_price_list(path: str = "price_list.xlsx") -> list[LabTest]:
    ...
    if missing:
        raise RuntimeError(f"В price_list.xlsx не хватает колонок: {', '.join(sorted(missing))}")
    ...
    for row in rows[1:]:
        ...
        synonyms_raw = str(get_cell(row, "synonyms", ""))
