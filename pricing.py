...
def _find_by_alias_group(query: str, price_list: list[LabTest], max_candidates: int) -> list[CandidateItem]:
    ...
    group_items = _sort_group_items(group_items)[:max_candidates]

    # Для group-match уверенность условная: 100, потому что это ручная связка.
    return [labtest_to_candidate(item, 100) for item in group_items]
