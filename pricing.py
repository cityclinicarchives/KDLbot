...
# Словарь приоритетной фильтрации для коротких популярных запросов.
# Если пользователь вводит, например, "ОАК", сначала ищем только среди анализов,
# где в названии или синонимах есть указанные ключевые фразы.
PRIORITY_SYNONYMS = {
...
def load_price_list(path: str = "price_list.csv") -> list[LabTest]:
    ...
    with file_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        # short_name — необязательная колонка. Если она есть, используется только для текста кнопок.
...
def find_candidate_tests(
    ...
    Логика:
    1. Нормализуем пользовательский запрос.
    2. Если запрос есть в PRIORITY_SYNONYMS, сначала жестко фильтруем прайс
       по медицински правильным ключевым словам.
       Например: "ОАК" -> только анализы, где есть "клинический анализ крови"
       или "общий анализ крови".
    3. Считаем score для каждого анализа.
    4. Применяем жесткий фильтр минимальной уверенности min_score.
    5. Берем лучший score среди отфильтрованных вариантов.
    6. Возвращаем все варианты, которые не хуже лучшего больше чем на close_gap.
    7. Ограничиваем список max_candidates, чтобы не перегружать пациента.
    """
    ...
    if priority_keywords:
        ...
        for item in price_list:
            combined_text = normalize_text(
                item.name + " " + " ".join(item.synonyms)
            )

            if any(keyword in combined_text for keyword in priority_keywords):
                filtered_price_list.append(item)

        # Если по словарю приоритетов что-то нашли, дальше ищем только внутри
        # этого медицински релевантного подмножества. Если ничего не нашли,
        # не ломаем поиск и возвращаемся к обычному fuzzy-поиску по всему прайсу.
