...
def candidate_selection_keyboard(matched_items, candidate_limits: dict | None = None) -> InlineKeyboardMarkup:
    """
    Для первого неоднозначного пункта показывает короткие кнопки выбора.

    Важно: Telegram не умеет гарантированно переносить длинный текст внутри inline-кнопки.
    Поэтому полные названия анализов выводятся в сообщении, а кнопки короткие: «1 — 580 ₽».
    """
    rows = []
    ...
    for item_index, item in enumerate(matched_items):
        ...
        for candidate_index, candidate in enumerate(visible_candidates):
            # Кнопки специально короткие, чтобы не обрезались на экране телефона.
            text = f"{candidate_index + 1} — {candidate.price} ₽"
            rows.append([
                InlineKeyboardButton(
                    text=text,
