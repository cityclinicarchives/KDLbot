...
def _format_components(components: str) -> str:
    text = str(components or "").strip()
    if not text:
        return ""
    ...
    if not parts:
        return ""
    if len(parts) <= 8:
        return "   Состав: " + "; ".join(parts)
    return "   Состав: " + "; ".join(parts[:8]) + f"; ... еще {len(parts) - 8}"
...
def format_preview_message(items) -> str:
    ...
    for index, item in enumerate(items, start=1):
        if item.status == "found":
            ...
            components_line = _format_components(getattr(item, "components", ""))
            if components_line:
...
def format_candidate_message(item_index: int, item, visible_limit: int = 4) -> str:
    ...
    for index, candidate in enumerate(candidates, start=1):
        ...
        components_line = _format_components(getattr(candidate, "components", ""))
        if components_line:
            lines.append(components_line)
...
def format_final_calculation(calculated_items, patient_discount: int) -> str:
    ...
    for index, item in enumerate(calculated_items, start=1):
        if item["status"] == "found":
            ...
            components_line = _format_components(item.get("components", ""))
            if components_line:
                lines.append(components_line)
        elif item["status"] == "included":
            lines.append(
                f"{index}. {item['name']} — входит в комплекс «{item['included_in_complex']}»"
            )
        else:
            lines.append(
...
def format_order_for_lab(order_id: str, patient_name: str, phone: str, patient_discount: int, calculated_items, ai_usage=None) -> str:
    ...
    for index, item in enumerate(calculated_items, start=1):
        if item["status"] == "found":
            ...
            lines.append(
                f"{index}. {item.get('code') or ''} — {item['name']} — входит в комплекс «{item['included_in_complex']}»"
            )
        else:
            lines.append(
    ...
    if ai_usage and ai_usage.get("used"):
        lines.extend([
            "",
            "ИИ-слой:",
            f"Источник: {ai_usage.get('source') or 'не указано'}",
            f"Prompt/Input tokens: {int(ai_usage.get('prompt_tokens') or 0)}",
            f"Completion/Output tokens: {int(ai_usage.get('completion_tokens') or 0)}",
            f"Total tokens: {int(ai_usage.get('total_tokens') or 0)}",
        ])
